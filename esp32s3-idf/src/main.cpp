...
extern "C" void app_main(void)
{
    ...
    while (true) {
        ...
        led_tick++;
        if (cmsis_dap::is_connected()) {
            // Target connected: green solid ON, red OFF
            board_config::set_led_green(true);
            board_config::set_led_red(false);
        } else if (tud_mounted()) {
            // USB enumerated, no target: green slow blink (500ms)
            if (led_tick % 5 == 0) {
            ...
            // USB not enumerated: red slow blink (500ms)
            if (led_tick % 5 == 0) {
                led_blink_state = !led_blink_state;
                board_config::set_led_red(led_blink_state);
            }
