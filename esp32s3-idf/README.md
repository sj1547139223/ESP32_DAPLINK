# ESP32-S3 DAPLink v1.1.0 — CMSIS-DAP v2 调试器
...
| 版本 | 修改内容 |
|------|----------|
| **v1.1.0** | 新增多芯片家族支持（CW32/GD32/CH32/APM32/AGM32）内置 Flash 算法；修复 CW32 芯片误识别为 STM32 的问题；修复 CW32 烧录数据错误（字对齐写入）；修复 WDT 在 FLM Init 等待时复位的问题；LED 改为 active-low 驱动，红色常亮表示运行中，绿色慢闪表示 WiFi 已连接，红绿交替快闪表示烧录/调试中 |
...
LED 为低电平点亮（active-low）。

| 优先级 | 条件 | 🔴 红灯 | 🟢 绿灯 |
|--------|------|---------|--------|
| 1 | 烧录中 / CMSIS-DAP 调试中 | 交替快闪（200ms） | 交替快闪（200ms） |
| 2 | 待机，WiFi 已连接 | 常亮 | 慢闪（500ms） |
| 3 | 待机，WiFi 未连接 | 常亮 | 灭 |
...
| **GD32F1** | F103 全系列（DEV_ID=0x410，REV_ID=0x1303/0x2303 识别） |
| **CH32F1** | V103 全系列（DEV_ID=0x410，REV_ID=0x2000 识别） |
| **APM32F1** | F103 全系列（Geehy，DEV_ID=0x410，REV_ID=0x1000/0x3000 识别） |
| **CW32** | L011 及全系列 Cortex-M0+（内置直驱 Flash 控制器，无需 FLM） |
| **AGM32** | 全系列（通过 STM32F4 算法兼容） |
