...
// ==========================================================================
// CW32 Flash Controller (CW32F030/F031/F036/F072/L083)
// Reference: CW32F030 Reference Manual
// Flash base: 0x40022000, Keys differ from STM32
// ==========================================================================

namespace cw32 {

constexpr uint32_t kFlashBase = 0x40022000;
constexpr uint32_t kFlashCR1  = kFlashBase + 0x00;  // Control Register 1 (unlock key)
constexpr uint32_t kFlashCR2  = kFlashBase + 0x04;  // Control Register 2 (erase/program)
constexpr uint32_t kFlashSR   = kFlashBase + 0x0C;  // Status Register
constexpr uint32_t kFlashAR   = kFlashBase + 0x10;  // Address Register

// CW32 unlock keys (different from STM32!)
constexpr uint32_t kKey1 = 0xCA000000;
constexpr uint32_t kKey2 = 0x35000000;

// CR2 bits
constexpr uint32_t kCR2_PG   = (1 << 0);   // Page programming enable
constexpr uint32_t kCR2_PER  = (1 << 1);   // Page erase
constexpr uint32_t kCR2_STRT = (1 << 6);   // Start erase
constexpr uint32_t kCR2_LOCK = (1 << 7);   // Lock

// SR bits
constexpr uint32_t kSR_BSY  = (1 << 0);
constexpr uint32_t kSR_EOP  = (1 << 5);

constexpr uint32_t kPageSize   = 512;  // CW32F030: 512 bytes/page
constexpr uint32_t kFlashStart = 0x08000000;
...
esp_err_t wait_not_busy(uint32_t timeout_ms = 2000)
{
    ...
    while (true) {
        ...
        if (!(sr & kSR_BSY)) return ESP_OK;
        if (esp_log_timestamp() - start > timeout_ms) {
            ESP_LOGE(kTag, "CW32 Flash busy timeout SR=0x%08lx", (unsigned long)sr);
            return ESP_ERR_TIMEOUT;
        }
        esp_rom_delay_us(100);
    }
...
esp_err_t unlock()
{
    // Write keys to CR1 to unlock CR2
    ESP_RETURN_ON_ERROR(mem_write32(kFlashCR1, kKey1), kTag, "CW32 key1 failed");
    ESP_RETURN_ON_ERROR(mem_write32(kFlashCR1, kKey2), kTag, "CW32 key2 failed");
    return ESP_OK;
}
...
{
    uint32_t cr2 = 0;
    mem_read32(kFlashCR2, cr2);
    return mem_write32(kFlashCR2, cr2 | kCR2_LOCK);
}

esp_err_t erase_page(uint32_t address)
{
    ESP_RETURN_ON_ERROR(wait_not_busy(), kTag, "busy before erase");
    ESP_RETURN_ON_ERROR(mem_write32(kFlashCR2, kCR2_PER), kTag, "PER set failed");
    ESP_RETURN_ON_ERROR(mem_write32(kFlashAR, address), kTag, "AR write failed");
    ESP_RETURN_ON_ERROR(mem_write32(kFlashCR2, kCR2_PER | kCR2_STRT), kTag, "STRT failed");
    ESP_RETURN_ON_ERROR(wait_not_busy(5000), kTag, "erase timeout");
    ESP_RETURN_ON_ERROR(mem_write32(kFlashCR2, 0), kTag, "CR2 clear failed");
    return ESP_OK;
}

} // namespace cw32

esp_err_t program_cw32(const target_probe::TargetInfo &target, const std::vector<hex_parser::Segment> &segments)
{
    ESP_LOGI(kTag, "CW32 flash programming: %u segments", (unsigned)segments.size());

    ESP_RETURN_ON_ERROR(ap_init_word_access(), kTag, "AP init failed");
    ESP_RETURN_ON_ERROR(halt_core(), kTag, "halt failed");
    ESP_RETURN_ON_ERROR(cw32::unlock(), kTag, "flash unlock failed");

    // Erase pages
    for (const auto &seg : segments) {
        uint32_t page_addr = seg.address & ~(cw32::kPageSize - 1);
        uint32_t end_addr = seg.address + seg.data.size();
        for (; page_addr < end_addr; page_addr += cw32::kPageSize) {
            ESP_LOGI(kTag, "  Erasing page at 0x%08lx", (unsigned long)page_addr);
            ESP_RETURN_ON_ERROR(cw32::erase_page(page_addr), kTag, "erase failed");
            vTaskDelay(1);
        }
    }

    // Program words (CW32 minimum write unit: 32-bit word, word-aligned)
    for (const auto &seg : segments) {
        ESP_RETURN_ON_ERROR(cw32::wait_not_busy(), kTag, "busy before program");
        ESP_RETURN_ON_ERROR(mem_write32(cw32::kFlashCR2, cw32::kCR2_PG), kTag, "PG set failed");

        for (size_t i = 0; i < seg.data.size(); i += 4) {
            uint32_t word = 0xFFFFFFFF;
            for (size_t b = 0; b < 4 && (i + b) < seg.data.size(); ++b)
                word = (word & ~(0xFFU << (b * 8))) | ((uint32_t)seg.data[i + b] << (b * 8));
            ESP_RETURN_ON_ERROR(mem_write32(seg.address + i, word), kTag, "write failed");
            ESP_RETURN_ON_ERROR(cw32::wait_not_busy(), kTag, "program timeout");
        ...
        ESP_RETURN_ON_ERROR(mem_write32(cw32::kFlashCR2, 0), kTag, "PG clear failed");
    }

    ESP_RETURN_ON_ERROR(cw32::lock(), kTag, "flash lock failed");
    ESP_RETURN_ON_ERROR(reset_and_run(), kTag, "reset failed");

    ESP_LOGI(kTag, "CW32 programming complete");
    return ESP_OK;
}
...
SelectionResult select_algorithm(const hex_parser::ParsedHexImage &image, const target_probe::TargetInfo &target)
{
    ...
    if (target.family == target_probe::Family::kStm32) {
        ...
        // STM32F2/F4 series
        if (dev == 0x411 || dev == 0x413 || dev == 0x419 || dev == 0x423 || dev == 0x433 ||
            dev == 0x431 || dev == 0x441 || dev == 0x421 || dev == 0x434 ||
            dev == 0x449 || dev == 0x451 || dev == 0x450 || dev == 0x480) {
            result.algorithm_name = "stm32f4_direct";
            return result;
        }
    ...
    // GD32: mostly STM32-compatible Flash controllers
    if (target.family == target_probe::Family::kGd32) {
        uint32_t dev = target.dev_id;
        // GD32F3xx/F1xx/E10x use STM32F1-compatible flash
        // GD32F4xx use STM32F4-compatible flash
        if (dev == 0x413 || dev == 0x419) {
            result.algorithm_name = "stm32f4_direct";  // GD32F4xx
        } else {
            result.algorithm_name = "stm32f1_direct";  // GD32F1xx/F3xx/E1xx
        }
        return result;
    }

    // CH32: STM32F1-compatible flash (CH32F103)
    if (target.family == target_probe::Family::kCh32) {
        result.algorithm_name = "stm32f1_direct";
        return result;
    }

    // APM32: STM32-compatible flash
    if (target.family == target_probe::Family::kApm32) {
        uint32_t dev = target.dev_id;
        if (dev == 0x413 || dev == 0x419) {
            result.algorithm_name = "stm32f4_direct";  // APM32F4xx
        } else {
            result.algorithm_name = "stm32f1_direct";  // APM32F1xx
        }
        return result;
    }

    // CW32: custom flash controller
    if (target.family == target_probe::Family::kCw32) {
        result.algorithm_name = "cw32_direct";
        return result;
    }

    // AGM32: STM32F4-compatible flash (AGM32PF100 based on Cortex-M4)
    if (target.family == target_probe::Family::kAgm32) {
        result.algorithm_name = "stm32f4_direct";
        return result;
    }

    // Nordic nRF
    if (target.family == target_probe::Family::kNordic) {
                         ...
                         const std::vector<hex_parser::Segment> &segments)
{
    ...
    if (selection.family == target_probe::Family::kStm32) {
        ...
        } else if (strstr(selection.algorithm_name, "f4") != nullptr) {
            // STM32F4 flash uses sector-based erase, not implemented as direct yet
            // Fall through to error; use FLM-based programming for F4
            ESP_LOGE(kTag, "STM32F4 direct programming not yet implemented, use FLM");
            return ESP_ERR_NOT_SUPPORTED;
        } else {
    ...
    // GD32/CH32/APM32: use STM32-compatible algorithms
    if (selection.family == target_probe::Family::kGd32 ||
        selection.family == target_probe::Family::kCh32 ||
        selection.family == target_probe::Family::kApm32) {
        if (strstr(selection.algorithm_name, "f4") != nullptr) {
            ESP_LOGE(kTag, "F4-compatible direct programming not yet implemented, use FLM");
            return ESP_ERR_NOT_SUPPORTED;
        }
        return program_stm32_f1(target, segments);
    }

    // CW32: custom controller
    if (selection.family == target_probe::Family::kCw32) {
        return program_cw32(target, segments);
    }

    // AGM32: F4-compatible (not implemented as direct yet)
    if (selection.family == target_probe::Family::kAgm32) {
        ESP_LOGE(kTag, "AGM32 direct programming not yet implemented, use FLM");
        return ESP_ERR_NOT_SUPPORTED;
    }
