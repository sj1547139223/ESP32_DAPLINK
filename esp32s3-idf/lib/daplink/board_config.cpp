...
void configure_led_gpio()
{
    ...
    gpio_set_level(kLedRedPin, 1);    // active-low: 1 = off
    gpio_set_level(kLedGreenPin, 1);  // active-low: 1 = off
}

void set_led_red(bool on)
{
    gpio_set_level(kLedRedPin, on ? 0 : 1);  // active-low
}

void set_led_green(bool on)
{
    gpio_set_level(kLedGreenPin, on ? 0 : 1);  // active-low
}

static ActivityState s_activity_state = ActivityState::kIdle;

void set_activity_state(ActivityState state)
{
    s_activity_state = state;
}
